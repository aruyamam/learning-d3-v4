...
export default async function lifeExpectancyTable() {
  ...
  return tableFactory(data).table
    .selectAll('tr')
    .filter(i => i)
    .sort(([coutnryA, yearsA], [coutryBj, yearsB]) => yearsA - yearsB);
}
...
export async function renderSVGStuff() {
  ...
  const rect = chart.container.insert('rect', ':first-child')
    .attr('x', 200)
    .attr('y', 50)
    .attr('width', 300)
    .attr('height', 400)
